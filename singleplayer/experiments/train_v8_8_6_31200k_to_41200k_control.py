...
# CONTROL EXPERIMENT (2026-08-29)
# Base: V8.8.6 formal Champion at 31.2M transitions.
# Goal: +10M transitions with the SAME production recipe, reaching 41.2M.
# No reward/gate/network/TD/replay/Teacher/candidate-contract changes.
# The 31.2M checkpoint is never overwritten.
...
try:
    ...
    from singleplayer.learning.cuda_graph import (
...
def vector_generator_loop(
    ...
    try:
        ...
        try:
            from singleplayer.backend.jax.vector_env import reset_batch
            from singleplayer.backend.jax.teacher import (
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "V8.8.6 CONTROL: continue the formal 31.2M Champion for exactly "
            "+10M transitions with the same 3-producer affinity/shared-weight "
            "CUDA-Graph DDQN recipe. No algorithmic changes."
        )
    )
    ...
    parser.add_argument(
        ...
        default=10000000,
        help=(
            "NEW transitions. Default +10,000,000 takes 31.2M -> 41.2M."
        ),
    )
    parser.add_argument(
        "--start-seed",
    ...
    parser.add_argument(
        ...
        default=2034892800,
        help=(
            "Preserves the V8.8.6 production ratio of 203.48928 replay samples "
            "per new transition for +10,000,000 transitions = 248,400 gradients "
            "at batch 8192."
        ),
    )
    ...
    parser.add_argument("--checkpoint-every", type=int, default=3000000)
    parser.add_argument(
        "--checkpoint-prefix",
        default="models/v8_8_6_control_continued_td",
    )
    parser.add_argument(
        "--max-batch-fraction",
        type=float,
    ...
    parser.add_argument(
        "--output",
        default="models/v8_8_6_control_continued_td_41200k.pt",
    )
    parser.add_argument(
        "--parity-stamp",
    ...
    if args.steps == 10_000_000 and args.sample_budget != 2_034_892_800:
        raise RuntimeError(
            "CONTROL RECIPE MISMATCH: +10M requires --sample-budget 2034892800 "
            "to preserve 203.48928 samples/new-transition."
        )
    ...
    print()
    print("=" * 80)
    print("V8.8.6 CONTROL CONTINUATION SUMMARY")
    print("=" * 80)
    print()
    print("Inherited transitions:", inherited_env_steps)
    ...
    if interrupted:
        print()
        print("V8.8.6 TRAINING: SAFELY INTERRUPTED")
        return
    ...
    print()
    print("V8.8.6 CONTROL 31.2M -> 41.2M SAME-RECIPE TRAINING: PASS")
    print(
        "NOTE: this 41.2M checkpoint is a CONTROL CHALLENGER only. The formal "
        "31.2M Champion remains unchanged until fresh evaluation/calibration and "
        "qualification justify promotion."
    )
