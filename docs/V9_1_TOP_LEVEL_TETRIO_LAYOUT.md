...
```bat
git rm tetris_ai\battle\rotation\tetrio_srs_plus.py
git rm tetris_ai\battle\rules\tetrio.py
...
.venv\Scripts\python.exe -m unittest discover -s tetris_ai\battle\tests -t . -v
.venv\Scripts\python.exe -m compileall -q tetris_ai\battle tetrio
```
