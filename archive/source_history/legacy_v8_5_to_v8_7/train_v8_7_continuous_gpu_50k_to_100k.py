    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            if random_alt:
                ...
                chosen_index, q_margin = normalized_margin_choice(
                    q_values,
                    behavior_gate,
                )
...
def save_checkpoint(
    *,
    ...
    torch.save(
        {
            "version": (
                "V8_7_NORMALIZED_GATE_OBSERVABLE_SAFE_"
                "CONTINUOUS_GPU_DDQN"
            ),
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "V8.7 continuous normalized-gate GPU training from the 50K warm-start "
            "to 100K, saving exact 10K checkpoints without re-tuning the gate."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=50000,
        help="NEW transitions to add (50K -> 100K by default).",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.600,
        help="Frozen V8.7 normalized confidence gate; do not re-tune per checkpoint.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.400,
        help="Aggressive normalized gate used only by risk actors to harvest failure data.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10178560,
        help="Approximate total replay samples to process in this 50K continuation.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10000,
        help="Save an exact training checkpoint every N NEW transitions; 0 disables periodic checkpoints.",
    )

    parser.add_argument(
        "--checkpoint-prefix",
        default="models/v8_7_normalized_gpu_td",
        help="Periodic checkpoint prefix; total transition count is appended as _60k.pt, _70k.pt, ...",
    )
    ...
    if args.checkpoint_every > 0 and args.checkpoint_every > args.steps:
    ...
    freshness_limit = max(
        1,
        int(math.floor(freshness_basis * args.max_batch_fraction)),
    )
    ...
    if int(chosen_batch_size) > freshness_limit:
        ...
        print(
            ...
            f"(batch <= {args.max_batch_fraction:.2f} * {freshness_basis} checkpoint-window transitions)",
        )
    ...
    print()
    print("=" * 80)
    print("V8.7 NORMALIZED-GATE OBSERVABLE-SAFE CONTINUOUS GPU TRAINING")
    print("=" * 80)
    print()
    print("Device:", device)
    ...
    print(
        ...
        (
            f"every {args.checkpoint_every} new transitions -> {args.checkpoint_prefix}_<TOTAL>k.pt"
            if args.checkpoint_every > 0
            else "DISABLED"
        ),
    )
    ...
    print("  CUDA batch 4096/8192/16384/32768 benchmarked at startup")
    print("  fixed replay-sample budget across batch choices")
    print("  freshness guard caps deployed batch relative to each checkpoint window")
    print("  exact 10K periodic checkpoints are saved during continuous training")
    print("  weight sync every 128 transitions")
    print("  queue 384 to reduce actor-policy staleness")
    print("  2 risk actors @ normalized gate 0.40 to harvest failure data")
    print("  post-step encode_state removed except periodic identity audits")
    print("  CUDA learner remains FP32; normalized gate is scale-invariant and frozen")
    ...
    try:
        while collected < args.steps:
            ...
            for message in messages:
                ...
                # Stop exactly on each checkpoint boundary rather than
                # overshooting inside a drained message batch.
                if (
                    next_checkpoint_step is not None
                    and collected >= next_checkpoint_step
                ):
                    break
            ...
            if (
                ...
                total_at_checkpoint = inherited_env_steps + collected
                checkpoint_k = total_at_checkpoint // 1000
                checkpoint_path = f"{args.checkpoint_prefix}_{checkpoint_k}k.pt"
                ...
                print(
                    ...
                    checkpoint_path,
                    f"(total transitions={total_at_checkpoint}, gradients={new_gradient_steps})",
                )

                next_checkpoint_step += args.checkpoint_every

                if next_checkpoint_step > args.steps:
    ...
    print()
    print("=" * 80)
    print("V8.7 NORMALIZED-GATE OBSERVABLE-SAFE CONTINUOUS GPU TRAINING SUMMARY")
    print("=" * 80)
    print()
    ...
    if args.checkpoint_every > 0:
        expected_periodic = args.steps // args.checkpoint_every
        if len(periodic_checkpoints) != expected_periodic:
            raise RuntimeError(
                f"Periodic checkpoint count mismatch: "
                f"{len(periodic_checkpoints)} != {expected_periodic}"
            )
        for checkpoint_path in periodic_checkpoints:
            if not os.path.isfile(checkpoint_path):
                raise RuntimeError(
                    f"Periodic checkpoint was not created: {checkpoint_path}"
                )
    ...
    print("Periodic Checkpoints    : PASS" if args.checkpoint_every > 0 else "Periodic Checkpoints    : DISABLED")
    print()
    print("V8.7 NORMALIZED-GATE CONTINUOUS GPU TRAINING: PASS")
