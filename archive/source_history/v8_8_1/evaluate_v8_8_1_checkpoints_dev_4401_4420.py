...
MODEL_SPECS = [
    (CHAMPION_LABEL, "models/v8_8_jax_vectorized_td_150k.pt", 0.600),
    ("v8_8_1_160k_norm", "models/v8_8_1_longtraj_gpu_replay_td_160k.pt", 0.600),
    ("v8_8_1_170k_norm", "models/v8_8_1_longtraj_gpu_replay_td_170k.pt", 0.600),
    ("v8_8_1_180k_norm", "models/v8_8_1_longtraj_gpu_replay_td_180k.pt", 0.600),
    ("v8_8_1_190k_norm", "models/v8_8_1_longtraj_gpu_replay_td_190k.pt", 0.600),
    ("v8_8_1_200k_norm", "models/v8_8_1_longtraj_gpu_replay_td_200k.pt", 0.600),
]
...
def _run_game(task):
    ...
    while pieces < max_pieces:
        ...
        max_height = max(max_height, _height_from_features(state_features))
...
def protect_seeds(seeds):
    bad = [
        s for s in seeds
        if PERMANENT_BENCHMARK_FIRST <= s <= PERMANENT_BENCHMARK_LAST
    ]
    if bad:
        raise RuntimeError(
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "V8.8.1 development-only checkpoint sweep on seeds 4401~4420. "
            "Use only to choose ONE V8.8.1 checkpoint for later untouched qualification."
        )
    )
    parser.add_argument("--seed-start", type=int, default=4401)
    parser.add_argument("--games", type=int, default=20)
    parser.add_argument("--max-pieces", type=int, default=2000)
    parser.add_argument("--workers", type=int, default=0)
    parser.add_argument(
        "--cache",
        default="data/v8_8_1_checkpoint_dev_4401_4420.json",
    )
    ...
    missing_files = [
        path for _, path, _ in MODEL_SPECS
        if not Path(path).exists()
    ]
    if missing_files:
        raise FileNotFoundError(
            "Missing checkpoint(s): " + ", ".join(missing_files)
        )

    workers = args.workers if args.workers > 0 else auto_workers()

    policies = [("teacher", None)] + [
        (label, gate)
        for label, _, gate in MODEL_SPECS
    ]
    ...
    if cache_path.exists():
        ...
        if cache.get("meta") != meta:
            raise RuntimeError(
                "Cache metadata mismatch; use a new --cache path."
            )
    ...
    missing = [
        ...
        if result_key(t["seed"], t["policy"])
        not in cache["results"]
    ]

    print("=" * 80)
    print("V8.8.1 DEVELOPMENT CHECKPOINT SWEEP — 4401..4420")
    print("=" * 80)
    print("Purpose: choose ONE V8.8.1 checkpoint; NOT qualification.")
    print("Formal Champion: V8.8 150K @ normalized gate 0.600")
    print("V8.8.1 gates: normalized 0.600")
    print("Seeds:", seeds)
    ...
    if missing:
        ...
        with ProcessPoolExecutor(
            ...
            for future in as_completed(futures):
                ...
                cache["results"][
                    result_key(result["seed"], result["policy"])
                ] = result
                ...
                if (
                    ...
                    or done % max(1, len(missing) // 20) == 0
                ):
    ...
    print()
    print("PAIRED VALUE VS FORMAL 150K CHAMPION")
    ...
    # lowest gameovers -> highest R/1000 -> highest average value.
    ranked = sorted(
        challenger_labels,
        key=lambda label: (
    ...
    print("=" * 80)
    print("Selected V8.8.1 checkpoint:", selected)
    print(
        "Selection rule: lowest gameovers, then highest R/1000, "
        "then highest average value."
    )
    print(
        "IMPORTANT: 4401~4420 are now DEVELOPMENT-CONSUMED. "
        "Do not use them for formal qualification or later tuning."
    )
    print(
        "Next: qualify only the selected checkpoint against the formal "
        "V8.8 150K Champion on one untouched predeclared block "
        "(recommended 3201~3220)."
    )
