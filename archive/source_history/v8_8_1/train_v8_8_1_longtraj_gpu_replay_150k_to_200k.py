...
from v8_8_1_train_common import (
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "V8.8.1: long-trajectory JAX rollout + packed device-resident replay "
    ...
    parser.add_argument(
        ...
        help=(
            "Warm-start checkpoint; default is the formal V8.8 150K Champion."
        ),
    )
    parser.add_argument(
        "--steps",
        type=int,
        default=50000,
        help="NEW transitions; default continues formal V8.8 Champion 150K -> 200K.",
    )
    parser.add_argument(
        "--start-seed",
        type=int,
        default=12001,
        help="V8.8.1 training stream seed base; kept disjoint from all evaluation blocks.",
    )
    ...
    parser.add_argument(
        "--checkpoint-prefix",
        default="models/v8_8_1_longtraj_gpu_replay_td",
    )
    parser.add_argument(
        "--max-batch-fraction",
        type=float,
    ...
    runtime_meta = {
        ...
        "formal_input_champion": "V8.8 150K normalized 0.600",
    ...
    print()
    print("=" * 80)
    print("V8.8.1 LONG-TRAJECTORY + PACKED DEVICE-REPLAY TRAINING")
    print("=" * 80)
    print()
    ...
    print("Input status:", "FORMAL V8.8 150K CHAMPION")
    ...
    def _learner_step():
        ...
        collect_metrics = metric_tracker.should_collect(new_gradient_steps + 1)
        metric_tensor = train_batch_device_replay(
            model=model,
    ...
    try:
        while collected < args.steps:
            if stop_event.is_set() and not generator.is_alive():
                raise RuntimeError(
                    "V8.8.1 generator stopped before collection completed."
                )
    ...
    print()
    print("V8.8.1 LONG-TRAJECTORY GPU-REPLAY TRAINING: PASS")
    print(
