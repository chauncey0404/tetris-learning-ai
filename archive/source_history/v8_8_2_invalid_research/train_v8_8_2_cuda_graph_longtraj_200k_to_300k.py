...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "V8.8.2: long-trajectory JAX rollout + packed device replay + CUDA Graph "
    ...
    parser.add_argument(
        ...
        help=(
            "Warm-start checkpoint; default is the failed-but-retained V8.8.1 200K challenger lineage. Formal Champion remains V8.8 150K."
        ),
    )
    parser.add_argument(
        "--steps",
        type=int,
        default=100000,
        help="NEW transitions; default continues challenger lineage 200K -> 300K.",
    )
    parser.add_argument(
        "--start-seed",
        type=int,
        default=13001,
        help="V8.8.2 training stream seed base; kept disjoint from all evaluation blocks.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Long-trajectory JAX streams. 32 preserves >5K generator states/s while reaching late game.",
    )
    parser.add_argument(
        "--risk-streams",
        type=int,
    ...
    parser.add_argument(
        ...
        default=20348928,
        help=(
            "Preserves the same ~203.49 replay samples/new transition ratio while doubling this run to 100K new transitions. "
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Small queue for low policy lag; with 32 streams two batches are sufficient.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--checkpoint-prefix",
        default="models/v8_8_2_cuda_graph_longtraj_td",
    )
    parser.add_argument(
        "--max-batch-fraction",
        type=float,
    ...
    parser.add_argument(
        "--output",
        default="models/v8_8_2_cuda_graph_longtraj_td_300k.pt",
    )
    parser.add_argument(
        "--parity-stamp",
    ...
    parser.add_argument(
        "--graph-gate-stamp",
        default="data/v8_8_2_cuda_graph_production_gate_pass.json",
    ...
    runtime_meta = {
        ...
        "input_lineage": "V8.8.1 200K failed formal qualification but retained as challenger training warm-start",
    ...
    print()
    print("=" * 80)
    print("V8.8.2 CUDA-GRAPH LONG-TRAJECTORY TRAINING")
    print("=" * 80)
    print()
    ...
    print(
        ...
        f"{args.steps / args.vector_envs:.1f}",
    )
    ...
    print("Input lineage:", "V8.8.1 200K CHALLENGER (qualification FAIL)")
    print("Output status:", "CHALLENGER ONLY until fresh qualification")
    print("CUDA Graph learner:", "ENABLED")
    ...
    try:
        while collected < args.steps:
            if stop_event.is_set() and not generator.is_alive():
                raise RuntimeError(
                    "V8.8.2 generator stopped before collection completed."
                )
            ...
            try:
                ...
                if not generator.is_alive():
                    raise RuntimeError(
                        "V8.8.1 generator exited before collection completed."
                    )
                continue
    ...
    print()
    print("V8.8.2 CUDA-GRAPH LONG-TRAJECTORY TRAINING: PASS")
    print(
