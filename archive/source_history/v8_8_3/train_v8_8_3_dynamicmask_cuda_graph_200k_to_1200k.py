...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "V8.8.3: long-trajectory JAX rollout + dynamic-mask CUDA Graph "
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000000,
        help="NEW transitions; restart from valid V8.8.1 200K and train to total 1.2M.",
    )
    parser.add_argument(
        "--start-seed",
        type=int,
        default=14001,
        help="V8.8.3 training stream seed base; kept disjoint from all evaluation blocks.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "1,000,000 new transitions = 24,840 gradients at batch 8192."
        ),
    )
    ...
    parser.add_argument(
        "--checkpoint-prefix",
        default="models/v8_8_3_dynamicmask_cuda_graph_td",
    )
    parser.add_argument(
        "--max-batch-fraction",
        type=float,
    ...
    print()
    print("=" * 80)
    print("V8.8.3 DYNAMIC-MASK CUDA-GRAPH LONG-TRAJECTORY TRAINING")
    print("=" * 80)
    print()
    ...
    try:
        while collected < args.steps:
            if stop_event.is_set() and not generator.is_alive():
                raise RuntimeError(
                    "V8.8.3 generator stopped before collection completed."
                )
    ...
    print()
    print("V8.8.3 DYNAMIC-MASK CUDA-GRAPH LONG-TRAJECTORY TRAINING: PASS")
    print(
