...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "V8.8.5: dual-JAX-producer hardware saturation + dynamic-mask CUDA Graph "
    ...
    parser.add_argument(
        "--checkpoint",
        default="models/v8_8_3_dynamicmask_cuda_graph_td_1200k.pt",
        help=(
            "Warm-start from the valid V8.8.3 1.2M challenger. "
            "Formal Champion remains V8.8 150K until fresh qualification."
        ),
    )
    parser.add_argument(
        "--steps",
        ...
        default=20000000,
        help=(
            "NEW transitions; dual-producer saturation run "
            "continues clean V8.8.3 1.2M -> 21.2M."
        ),
    )
    parser.add_argument(
        "--start-seed",
        type=int,
        default=16001,
        help="V8.8.5 training stream seed base; disjoint from prior/evaluation blocks.",
    )
    ...
    parser.add_argument(
        ...
        default=4069785600,
        help=(
            "Preserves 203.48928 replay samples/new transition for "
            "20,000,000 new transitions = 496,800 gradients at batch 8192."
        ),
    )
    ...
    parser.add_argument("--checkpoint-every", type=int, default=2000000)
    parser.add_argument(
        "--checkpoint-prefix",
        default="models/v8_8_5_dual_producer_cuda_graph_td",
    )
    parser.add_argument(
        "--max-batch-fraction",
        type=float,
    ...
    parser.add_argument(
        "--output",
        default="models/v8_8_5_dual_producer_cuda_graph_td_21200k.pt",
    )
    parser.add_argument(
        "--parity-stamp",
    ...
    runtime_meta = {
        ...
        "formal_champion_reference": "V8.8 150K normalized 0.600",
        "input_lineage": "V8.8.1 200K VALID challenger; V8.8.2 210K..300K excluded due frozen-next_mask graph bug",
    ...
    print()
    print("=" * 80)
    print("V8.8.5 DUAL-PRODUCER CUDA-GRAPH TRAINING")
    print("=" * 80)
    print()
    ...
    print("Formal Champion reference:", "V8.8 150K normalized 0.600")
    print("Input lineage:", "V8.8.3 1.2M VALID CHALLENGER")
    print("Output status:", "CHALLENGER ONLY until fresh qualification")
    ...
    initial_cpu_state = cpu_state_dict(model)
    generators = []
    ...
    while len(ready_reports) < args.producers:
        ...
        if remaining <= 0:
            ...
            raise RuntimeError(
                "V8.8.5 JAX producers did not all become ready in time."
            )
        ...
        try:
            ...
            if dead:
                ...
                for generator in generators:
                    ...
                    generator.join(timeout=2.0)
                raise RuntimeError(
                    "V8.8.5 producer died during startup: "
                    + ", ".join(dead)
                )
            continue
    ...
    try:
        while collected < args.steps:
            if stop_event.is_set():
                ...
                if dead:
                    raise RuntimeError(
                        "V8.8.5 producer stopped before collection completed: "
                        + ", ".join(dead)
                    )
            ...
            try:
                ...
                if dead:
                    raise RuntimeError(
                        "V8.8.5 producer exited before collection completed: "
                        + ", ".join(dead)
                    )
                continue
    ...
    if interrupted:
        print()
        print("V8.8.5 TRAINING: SAFELY INTERRUPTED")
        return
    ...
    print("JAX Multi-Producer Rollout : PASS")
    ...
    print()
    print("V8.8.5 DUAL-PRODUCER CUDA-GRAPH TRAINING: PASS")
    print(
